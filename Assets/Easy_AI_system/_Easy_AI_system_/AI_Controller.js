...
function Start () 
{
 ...
 // Based on viewTrigger collider size and waypointActivationDistance specified in pathFollower
  rangedAttackDistance = GetMaxLookingDistance(viewTrigger.gameObject.collider);
	  ...
	  if (timer>0) 
		   {
		     ...
		     if(currentAction.animationClip)
		       if(!animatedObject.animation.isPlaying)
		          if (currentAction.loopAnimation) animatedObject.animation.CrossFade(currentAction.animationClip.name);
		             else timer=0;
		   }
    ...
    if (action.actionPriority > currentAction.actionPriority || timer <=0) 
     {
	    ...
	    if (animatedObject.animation && action.animationClip) animatedObject.animation.CrossFade(action.animationClip.name);
  ...
  if (enemy==runAwayObject) 
     ...
     Debug.Log("!!!");
      ...
      if (followAlly) ManageAction(GetActionByID(AI_ActionTypes.Follow));
		...
		if(!Physics.Raycast (transform.position, enemy.transform.position-transform.position, rangedAttackDistance))
		 { 
